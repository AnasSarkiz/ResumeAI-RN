...
export default function AIGeneratorScreen() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      Alert.alert('Success', 'Your AI resume is ready!', [
        {
          text: 'Preview',
          onPress: () => router.push({ pathname: '/resume/preview', params: { id: String(created.id) } }),
        },
        {
          text: 'Go to Home',
          style: 'cancel',
      ...
      console.error(err);
      Alert.alert('Generation failed', 'There was a problem generating your resume. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const FooterNav = () => (
    <View className="mt-6 flex-row gap-3">
      {step > 0 && (
        <TouchableOpacity onPress={handleBack} className="flex-1 flex-row items-center justify-center rounded-full border border-gray-300 bg-white py-3">
      ...
      {step < 5 ? (
        ...
        <TouchableOpacity onPress={handleGenerate} disabled={loading} className={`flex-1 flex-row items-center justify-center rounded-full py-3 ${loading ? 'bg-gray-300' : 'bg-green-600'}`}>
          {loading ? (
  ...
  return (
    <ScrollView className="flex-1 bg-gray-50">
      ...
      <LinearGradient colors={["#eef2ff", "#faf5ff"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={{ paddingHorizontal: 16, paddingBottom: 16, paddingTop: 24 }}>
        ...
        <Text className="text-sm text-gray-600">Fill the steps. On the last step, add how you want the design. We'll generate a complete resume.</Text>
        <View className="mt-4">
          <StepIndicator />
        </View>
      </LinearGradient>
      ...
      {step === 0 && (
        <View className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
          ...
          <EditableTextInput label="Full Name *" value={formData.fullName} onChange={(t) => handleInputChange('fullName', t)} required error={errors.fullName} />
          ...
          <View className="mt-2 flex-row items-end gap-3">
            <View style={{ width: 96 }}>
              <EditableTextInput
                ...
                onChange={(t) => handleInputChange('countryCode', t.startsWith('+') ? t : `+${t}`)}
                placeholder="+1"
              />
            </View>
            <View className="flex-1">
          ...
          <Text className="mt-2 text-xs text-gray-500">Tip: Use your primary contact details. We won't share them.</Text>
                ...
                <View className="flex-row gap-3">
                  <View style={{ width: 120 }}>
                    <EditableTextInput
                      label={`Label ${idx + 1}`}
                      value={lnk.label}
                      onChange={(t) => setLinks((prev) => prev.map((v, i) => (i === idx ? { ...v, label: t } : v)))}
                  ...
                  <View className="flex-1">
                    <EditableTextInput
                      label="URL"
                      value={lnk.url}
                      onChange={(t) => setLinks((prev) => prev.map((v, i) => (i === idx ? { ...v, url: t } : v)))}
                      placeholder="https://linkedin.com/in/username"
                    />
                  </View>
                </View>
                {links.length > 1 && (
                  <TouchableOpacity onPress={() => setLinks((prev) => prev.filter((_, i) => i !== idx))} className="self-end rounded-full px-2 py-1">
                    <Text className="text-xs text-red-500">Remove</Text>
                  </TouchableOpacity>
                )}
              </View>
            ))}
            <TouchableOpacity onPress={() => setLinks((prev) => [...prev, { label: '', url: '' }])} className="mt-1 flex-row items-center">
      ...
      {step === 1 && (
        <View className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
          ...
          <EditableTextInput label="Current/Target Job Title" value={formData.jobTitle || ''} onChange={(t) => handleInputChange('jobTitle', t)} />
          <EditableTextInput label="Industry" value={formData.industry || ''} onChange={(t) => handleInputChange('industry', t)} placeholder="e.g., Technology, Healthcare" />
          <EditableTextInput label="Target Role" value={formData.targetRole || ''} onChange={(t) => handleInputChange('targetRole', t)} placeholder="e.g., Senior Software Engineer" />
          <EditableTextInput label="Professional Summary" value={formData.summary || ''} onChange={(t) => handleInputChange('summary', t)} multiline placeholder="Brief summary of your background" />
          <Text className="mt-2 text-xs text-gray-500">Keep it 2-4 sentences focusing on impact and strengths.</Text>
        </View>
      )}
        ...
        <View className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
          ...
          {educations.map((val, idx) => (
            <View key={`edu-${idx}`} className="mb-2">
              ...
              />
              {educations.length > 1 && (
                <TouchableOpacity onPress={() => setEducations((prev) => prev.filter((_, i) => i !== idx))} className="self-end rounded-full px-2 py-1">
                  <Text className="text-xs text-red-500">Remove</Text>
                </TouchableOpacity>
              )}
            </View>
          ))}
          <TouchableOpacity onPress={() => setEducations((prev) => [...prev, ''])} className="mt-1 flex-row items-center">
            <Ionicons name="add-circle" size={18} color="#6366f1" />
            <Text className="ml-1 text-sm font-medium text-indigo-600">Add education</Text>
          </TouchableOpacity>
          <Text className="mt-2 text-xs text-gray-500">Example: B.Sc. in Computer Science — ABC University (2019)</Text>
        </View>
      )}
          ...
          {experiences.map((val, idx) => (
            <View key={`exp-${idx}`} className="mb-2">
              ...
              {experiences.length > 1 && (
                <TouchableOpacity onPress={() => setExperiences((prev) => prev.filter((_, i) => i !== idx))} className="self-end rounded-full px-2 py-1">
                  <Text className="text-xs text-red-500">Remove</Text>
                </TouchableOpacity>
              )}
            </View>
          ))}
          <TouchableOpacity onPress={() => setExperiences((prev) => [...prev, ''])} className="mt-1 flex-row items-center">
            <Ionicons name="add-circle" size={18} color="#6366f1" />
            <Text className="ml-1 text-sm font-medium text-indigo-600">Add experience</Text>
          </TouchableOpacity>
          <Text className="mt-2 text-xs text-gray-500">Use action verbs and quantify results when possible.</Text>
        </View>
      )}
        ...
        <View className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
          ...
          {skillsList.map((val, idx) => (
            <View key={`skill-${idx}`} className="mb-2">
              ...
              />
              {skillsList.length > 1 && (
                <TouchableOpacity onPress={() => setSkillsList((prev) => prev.filter((_, i) => i !== idx))} className="self-end rounded-full px-2 py-1">
                  <Text className="text-xs text-red-500">Remove</Text>
                </TouchableOpacity>
              )}
            </View>
          ))}
          <TouchableOpacity onPress={() => setSkillsList((prev) => [...prev, ''])} className="mt-1 flex-row items-center">
            <Ionicons name="add-circle" size={18} color="#6366f1" />
            <Text className="ml-1 text-sm font-medium text-indigo-600">Add skill</Text>
          </TouchableOpacity>
          <Text className="mt-2 text-xs text-gray-500">Example: React, TypeScript, Node.js, AWS, CI/CD</Text>
        </View>
      )}
      ...
      {step === 5 && (
        <View className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
          ...
          />
          <Text className="mt-2 text-xs text-gray-500">Hint: Mention color accents, column layout, and typography vibe.</Text>
        </View>
      )}
