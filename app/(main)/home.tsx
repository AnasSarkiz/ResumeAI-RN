...
export default function HomeScreen() {
  ...
  const ResumeCard = ({ item, index = 0 }: { item: Resume; index?: number }) => {
    ...
    const isAI = (item as any).kind === 'ai' && !!(item as any).aiHtml;
    const onManualEdit = () => {
      if (isAI) {
        Alert.alert(
          'Manual edit disabled',
          'This resume was generated by AI. Please use AI-Edit to modify it.'
        );
        return;
      }
      router.push(`/resume/editor?id=${item.id}`);
    };
